...
class BadmintonAPI {
     ...
     * @param {boolean} skipAutoActive - Skip auto-updating active status (for admin edits)
     * @returns {Promise<object>} - { success }
     */
    async updateGameState(courtId, gameState, skipAutoActive = false) {
        const queryParam = skipAutoActive ? '?skipAutoActive=true' : '';
        return this.request(`/game-states/${courtId}${queryParam}`, {
            method: 'PUT',
