...
function nedskaeringPanel(ned) {
    ...
    const kort = ned.liste.map((f, i) => `
        <div class="ned-kort">
            ...
            ${f.aendringer.length ? `<table class="ned-tabel"><thead><tr><th>Kategori</th><th>Runder</th><th>Kampe</th><th>Sikret pr. spiller</th><th>Krav</th><th>Under kravet</th></tr></thead><tbody>${f.aendringer.map((a) => `<tr><td>${esc(a.kategori)}</td><td>${a.fra === a.til ? `${a.fra} <span class="daempet">(uændret)</span>` : `${a.fra} → <b>${a.til}</b>`}</td><td>${a.kampeFoer} → ${a.kampeEfter}</td><td>${a.faerrestFoer} → ${a.faerrestEfter}</td><td>${a.krav}</td><td class="${a.underKravEfter ? 'er-roed' : ''}">${a.underKravEfter} spillere</td></tr>`).join('')}</tbody></table>` : '<p class="daempet">Ingen runder skæres.</p>'}
